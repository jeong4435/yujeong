...
작성 규칙:
- **독자 수준 = 고등학생~대학교 2학년(투자 입문자)**. 쉬운 말과 비유로 풀어 쓰되, **쉽게 쓴다고 분석의 깊이·정확성·전문성을 낮추지 말 것** — 근거(숫자·데이터)는 구체적으로 인용하고, 두루뭉술하거나 뻔한 말로 채우지 말 것. (당신은 전문 애널리스트이되, 입문자에게 과외하듯 설명한다)
...
- **각 항목은 반드시 "▌제목 :: 한 줄 요약" 형식의 머리줄로 시작**하세요.
  · 제목과 한 줄 요약은 정확히 ' :: '(공백+콜론2개+공백)로 구분.
  · 한 줄 요약 = 그 섹션의 핵심을 25자 내외로 압축한 한 문장(상세 설명과 표현이 겹치지 않게).
  · 머리줄 다음 줄부터 상세 설명(3~5문장)을 씁니다.
  · 예) "▌밸류에이션 분석 :: 현재 PER은 높지만 예상 PER은 낮아 성장 기대 반영"
- 제목은 "밸류에이션 분석 / 펀더멘털 분석 / 최근 이슈 / 증권가 시각 / 종합 시각" 그대로 사용
- 한국어로 작성"""
...
작성 규칙:
- **독자 수준 = 고등학생~대학교 2학년(투자 입문자)**. 쉬운 말·비유로 풀되, 쉽게 쓴다고 **분석의 깊이·정확성·전문성을 낮추지 말 것**(근거는 구체적으로, 두루뭉술 금지).
- 전문용어는 처음 등장 시 괄호로 짧게 풀이
- 데이터에 없는 수치를 지어내지 말 것
- '지금 사세요/파세요' 같은 매수·매도 권유 금지
- **각 항목은 "▌제목 :: 한 줄 요약" 형식 머리줄로 시작**(제목과 요약을 ' :: '로 구분, 요약은 25자 내외 핵심 한 문장), 다음 줄부터 상세.
  · 예) "▌시황 :: 미국은 강세지만 국내는 약세, 위험 회피 분위기"
- 제목은 "시황 / 섹터" 그대로 사용
- 한국어로 작성"""
