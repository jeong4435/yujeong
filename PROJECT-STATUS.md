...
>
> **🎯 제품 방향·차별화(2026-06-22 확정)**: 토스증권 대비 주력 = **①개인 포트폴리오 AI 코칭**(내 잔고·투자유형 기반 진단·조언) + **②초보 눈높이 교육**. 정체성 = *"초보도 이해하고, 내 포트폴리오를 AI가 코칭해주는 학습 도구"*(거래는 토스, **이해·코칭은 주식도AI**). → 지금 잔고(holdings)→매매→포트폴리오 분석 라인이 곧 차별점 ①의 토대. 거래 불가·종가 기준은 한계로 인정, "보완재" 포지션.
...
14. **디자인 전면 리스킨(완료·배포됨)** — **흰 배경 + 바이올렛(#6C5CE7) 포인트 + 상승빨강(#F04452)/하락파랑(#3182F6)**, 무채색 한마디, 텍스트 워드마크(로고심볼 제거), 이슈카드 종목 아이콘 제거. `styles.css` 토큰 교체 + `App.jsx` 워드마크/탭 정리. 시안: `mockup-design.html`(최종), `mockup-nav.html`(내비).
...
5. 디자인: **흰 배경 + 바이올렛 포인트(#6C5CE7) + 상승빨강/하락파랑**, 무채색 "주식도AI 한마디", 텍스트 워드마크, 본문 강조는 연노란 형광펜. (토큰은 `styles.css`, 상세는 `CLAUDE.md` 디자인 시스템)
...
> **⭐ 우선순위 재조정(2026-06-21)**: 사용자가 **잔고(보유 주식) 저장을 로그인 후 1순위 기능**으로 명시. **투자유형 개인화 멘트는 중요도 낮음**으로 강등. → 아래 Phase는 **잔고(holdings) 먼저**, 투자유형은 뒤로.

| Phase | 무엇을 | 케이스 | 의존 | 난이도/리스크 |
|---|---|---|---|---|
| **0. 토대** | Supabase 셋업(DB + 로그인 + RLS) | b 토대 | — | 중 (셋업이 관건) |
| **1. 잔고(최우선)** | 로그인 + **보유종목 수동 입력**(종목·수량·평단) → "내 잔고" 화면(평가손익). `holdings` 표. 진입점=종목분석 하단 `MyStocks.jsx` | b | 0 | 중·효과 큼 |
| **2. 매매 히스토리** | 매수/매도 거래 입력 → **평단·수량 자동 갱신**(`transactions` 표 → holdings 재계산) | b | 1 | 중 |
| **3. 포트폴리오 조언** | **섹터 집중도·분산 진단**(잔고 기반) | b | 1~2 | 중·규제 주의 |
| **4. 투자유형 개인화(낮음)** | 퀴즈 유형을 `profiles`에 저장 → 분석 멘트 개인화 | b | 0 | 낮음·**중요도 낮음** |
