...
## 프론트엔드 (React 18 + Vite)
- `src/App.jsx` — 탭 4개(왼→오): **오늘의 시장**(기본·홈) / 이슈 종목 / 종목 분석 / **마이**. 워드마크 클릭=오늘의 시장. **우상단 헤더에 `AuthButton`(로그인/프로필)**.
- `src/components/MyPage.jsx` — **마이 허브**(2026-06-22): 서브탭 4개 — 내 잔고/매매 기록/투자 유형/설정. 잔고·매매·설정은 로그인 필요(비로그인 시 로그인 유도), **투자 유형은 비로그인도 가능**(`Quiz`). 종목분석에 있던 '내 보유주식'은 여기로 이동.
- `src/holdings.js` — **잔고/매매 데이터 계층**: `loadHoldings`·`loadTransactions`·`upsertHolding`(직접 등록/수정)·`registerTrade`(매수/매도→`transactions` 기록 + `holdings` 자동 갱신: 매수=가중평균/매도=차감·0이면 삭제)·`deleteHolding`·`deleteTransaction`·`resolveStock`·`fetchPrices`(현재가).
- `src/components/Holdings.jsx` — 내 잔고: 목록(평가손익 종가기준) + 등록폼(`TradeForm`: 매수/매도/잔고직접수정 모드) + 종목클릭→분석. `src/components/TxHistory.jsx`(매매 일지)·`Settings.jsx`(프로필·로그아웃).
...
- 폰트: Pretendard 단일
- 시그니처: 메모 박스 `.sa-note` + 라벨 **"주식도AI 한마디"** — **무채색(연회색)**. 텍스트 정체성 유지.
- **AI 분석 섹션 제목**(`.sa-analysis-label`): **연노란 칩(도형) 배경 `--mark #FFF3A0` + 진회색 글씨 `--ink2`**. `.sa-chip`(AI분석 칩)과 같은 알약 형태, 컬러만 노랑. 옆에 한 줄 요약(`.sa-analysis-sum`: 굵게 `--ink2`).
- **본문 속 섹터명 강조**(`.sa-sector-hl`): 텍스트는 본문과 동일(색·굵기 상속, `<mark>` 기본 검정은 `color:inherit`로 덮음) + **연보라 형광펜만 살짝**(accent-soft 마커).
- AI 분석 섹션 제목은 "제목 :: 한 줄 요약" 형식(`parseAnalysis`가 분리) → 제목=형광펜, 요약=`.sa-analysis-sum`(회색 한 줄).
- 헤더: 로고 심볼(`株`) 제거 → **텍스트 워드마크 `.sa-wordmark`**("주식도 AI", AI만 보라). 누르면 홈(오늘의 시장 탭).
- 톤: 흰 바탕·넓은 여백·큰 숫자·플랫(얇은 회색 테두리 카드). 강조색은 바이올렛(포인트)/빨강·파랑(등락)/연노란(형광펜), 나머지는 흰·회색.
- **어조·수준(규칙, 2026-06-22)**: 모든 사용자 노출 텍스트(특히 **Gemini AI 분석글**)는 **독자 = 고등학생~대학교 2학년(투자 입문자)** 수준. 쉬운 말·비유로 풀고 전문용어는 첫 등장 시 괄호 풀이. **단, 쉽게 쓴다고 분석의 깊이·정확성·전문성을 낮추지 말 것**(근거는 구체적으로, 두루뭉술 금지). 이 기준은 `explain.py`의 두 프롬프트(`_build_prompt`·`_build_market_prompt`)에 명시됨. ※차별점 ②(초보 눈높이 교육)의 핵심 실행 규칙.
