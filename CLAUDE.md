...
- 시그니처: 메모 박스 `.sa-note` + 라벨 **"주식도AI 한마디"** — **무채색(연회색)** 유지(노란 *박스*는 폐기). 텍스트 정체성 유지. ※주의: '노란 박스 폐기'는 메모 박스 한정. 아래 형광펜과 혼동 말 것.
- **본문 강조 형광펜(2026-06-21~)**: 줄글 속 강조(섹터명 등)·AI 분석 섹션 제목은 **연노란 형광펜 `--mark #FFF3A0` + 진한 회색 글씨 `--ink`**. 클래스 `.sa-sector-hl`(본문 강조)·`.sa-analysis-label`(섹션 제목). 앞으로 줄글 내 강조는 이 연노란 형광펜으로 통일(바이올렛 칩 아님 — AI분석 칩과 색 겹쳐 과함).
