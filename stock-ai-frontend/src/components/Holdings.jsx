...
import {
  loadHoldings, fetchPrices, registerTrade, upsertHolding, deleteHolding, resolveStock,
} from "../holdings.js";
...
function TradeForm({ onDone }) {
  ...
  const needPrice = mode !== "sell";   // 매도는 단가 선택(잔고 영향 없음) → 생략 허용
  const priceLabel = mode === "set" ? "평단(원)" : "단가(원)";
  ...
  return (
    <div className="sa-tradeform">
      <div className="sa-mode">
        {[["buy", "매수"], ["sell", "매도"], ["set", "잔고 직접수정"]].map(([m, label]) => (
          <button key={m} className={"sa-mode-b" + (mode === m ? " on" : "")} onClick={() => setMode(m)}>{label}</button>
        ))}
      </div>
      ...
      <div className="sa-traderow">
        ...
        {needPrice && (
          <input className="sa-input" type="number" inputMode="numeric" placeholder={priceLabel} value={price} onChange={(e) => setPrice(e.target.value)} />
        )}
      </div>
